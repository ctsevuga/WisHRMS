...
const createInput = async (req, res) => {
  try {
    ...
    const conversionCost = safeNumber(req.body.conversionCost); // ✅ new field
    
    // Materials validation
    if (!Array.isArray(materials) || materials.length === 0) {
      return res
        .status(400)
        .json({ message: "Materials array is required and cannot be empty." });
    }
    ...
    const enrichedMaterials = await Promise.all(
      materials.map(async (material) => {
        if (!material.Product || !material.qtyInKg) {
          throw new Error(
            `Material entry is missing Product or qtyInKg: ${JSON.stringify(
              material
            )}`
          );
        }
        ...
        if (typeof product.Price !== "number") {
          throw new Error(
            `Product Price is invalid for product ID: ${material.Product}`
          );
        }
    ...
    const totalMaterialCost = enrichedMaterials.reduce(
      (acc, item) => acc + item.cost,
      0
    );
    const totalMaterialInKg = enrichedMaterials.reduce(
      (acc, item) => acc + item.qtyInKg,
      0
    );
    ...
    // ✅ New calculation including conversionCost
    const costWithConversionKgPerCost = overallmaterialkgPerCost + conversionCost;
    ...
    const newInput = new Input({
      ...
      conversionCost, // ✅ save conversionCost
      costWithConversionKgPerCost, // ✅ save cost with conversion
    });
    ...
    res
      .status(201)
      .json({ message: "Input created successfully", input: newInput });
  } catch (error) {
